...
# Notable Features
- A `token limit` for both `prompt` and `response` can be set to control costs. In the event that the token limit is reached a comment will not be created. The default limit is `10000` prompt tokens and `256` response tokens. For PRs with lots of changes, this limit will most likely need to be increased.
